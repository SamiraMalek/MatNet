...
folder_path = '/mnt/6a35bb5c-3c75-400e-a2fa-146fee20967e/home/fcssaleh/Weighted_Belief_Propagation/Polar_128_96'
H = np.load(folder_path+'/H_Polar_128_96.npy')
...
batch_in_epoch = 2000
batches_for_val = np.array([10,20,30,50,100,200])
...
check_dict["c_l{0}".format(i)] = tf.multiply(mask2,check_dict["c_l{0}".format(i)]) + mask3 
...
for i in range(1,num_iteration):
    ...
    var_dict["v_l{0}".format(i)] = tf.reduce_sum(t,axis=1) 
    ...
    bias_out["b_out_l{0}".format(i)] = tf.Variable(np.ones([n_code],dtype=np.float32))
    weight_out["w_out_l{0}".format(i)] = tf.Variable(H)
    weight_out["w_out_l{0}".format(i)] = tf.multiply(H,weight_out["w_out_l{0}".format(i)])
    out["l{0}".format(i)] = tf.multiply(weight_out["w_out_l{0}".format(i)],check_dict["c_l{0}".format(i)])
    out["l{0}".format(i)] = tf.reduce_sum(out["l{0}".format(i)],axis=1) + tf.multiply(bias_out["b_out_l{0}".format(i)],x)
    
sig_out = tf.nn.sigmoid(out["l5"])
error = tf.cast(sig_out > 0.5,'float')
error = tf.math.abs(error - y_test)
BER_one_batch = tf.reduce_mean(error)

cost = {}
for i in range(1,num_iteration):
    cost["l{0}".format(i)] = tf.reduce_mean( tf.nn.sigmoid_cross_entropy_with_logits(logits=out["l{0}".format(i)],labels=y))
total_cost = cost["l1"]+ cost["l2"]+ cost["l3"]+ cost["l4"]+ cost["l5"]
train_step = tf.train.AdamOptimizer(learning_rate=0.001).minimize(total_cost)
...
with tf.Session() as sess:    
    ...
    y_train = np.zeros([batch_size,n_code], dtype=np.float32)
    Y_test = y_train
    vector_number = 0
    for i in range(6):
        BER_sum_batchs = 0
    ...
    print('SNR[dB] validation - ', snr_print)
    ...
    for i in range(num_of_batch):
        ...
        if(i%batch_in_epoch == 0):
            ...
            np.save('V2MN28_128_96.npy',BER[1:,:])
            pltf.figure()
            ...
            pltf.grid(True)
            pltf.savefig('V2MN28_128_96_sum_of_BER.png')
            pltf.close()
