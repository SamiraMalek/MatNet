...
snr_print = np.array([1,2,3,4,5,6,7,8])
...
with tf.Session() as sess:    
    ...
    for i in range(8):
        ...
        for j in range(batches_for_val[i]): 
            ...
            X_test = x_v[start:stop,:]
            Y_test = y_v[start:stop,:]
            BER_j = sess.run(fetches = BER_one_batch ,feed_dict={x:X_test,y_test:Y_test})
    ...
    for i in range(num_of_batch):
        ...
        if(i%batch_in_epoch == 0):
            ...
            pltf.grid(True)
            pltf.savefig('MN9BCH63_51sum_of_BER.png')
            pltf.close()
