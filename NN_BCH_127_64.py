...
x_v = np.load(folder_path+'/x_test.npy')
y_v = np.load(folder_path+'/y_test.npy')
...
snr_dBrange = np.array([5,5.5,6,6.5,7,7.5,8,8.5,9,9.5])
...
batch_in_epoch = 1000
batches_for_val = np.array([10,10,10,30,40,100,300,600])*40/200
...
with tf.Session() as sess:    
    ...
    for i in range(8):
        ...
        for j in range(batches_for_val[i]): 
            ...
            BER_j = sess.run(fetches = BER_one_batch ,feed_dict={x:x_v_j,y_test:y_v_j})
    ...
    for i in range(num_of_batch):
        ...
        if(i%batch_in_epoch == 0):
            ...
            np.save('MN5BCH127_64.npy',BER[1:,:])
            pltf.figure()
            ...
            pltf.grid(True)
            pltf.savefig('MN5BCH127_64_sum_of_BER.png')
            pltf.close()
